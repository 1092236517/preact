...
import { getPersonalRecruitList, getPost, getBroker, brokerShareBind, happenCounts, getWXShareParam, getShareText } from "@/services/detail";
...
const $store = observable({
  ...
  geListData: async type => {
    ...
    try {
      const res = await getPersonalRecruitList(query);
      if (res.Code === 0) {
        // 使用runInAction
        runInAction(() => {
          ...
          $store.isEnd = res.Data.RecordList.length < query.RecordSize ? true : false;
          window.ontouchmove="";
        })
      }
    } catch (e) {}
